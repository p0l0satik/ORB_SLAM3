...
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &mask, const cv::Mat &depthmap, const double &timestamp, string filename)
{
    if(mSensor!=RGBD)
    {
    ...
    cv::Mat Tcw = mpTracker->GrabImageRGBD(im, mask, depthmap, timestamp, filename);
